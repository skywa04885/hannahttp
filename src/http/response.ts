...
} from "./headers/transfer-encoding";
import {
  HTTPContentEncoding,
  HTTPContentEncodingHeader,
} from "./headers/content-encoding";
import { HTTPRequest } from "./request";
import { HTTPRangeHeader } from "./headers/range";
import {
  HTTPConnectionPreference,
  HTTPConnectionPreferenceHeader,
} from "./headers/connection";
import { HTTPContentTypeHeader, HTTPMediaType } from "./headers/content-type";
...
export class HTTPResponse extends EventEmitter {
  ...
  protected _contentType?: HTTPContentTypeHeader;
  ...
  public setContentType(
    ...
    this._contentType = new HTTPContentTypeHeader(mediaType, charset, boundary);
    return this;
  }
   ...
   * @returns the current instance.
   */
  public addBodyTransform(transform: Transform, raw = false): this {
    if (
      ![
  ...
  public async status(code: number, message?: string): Promise<void> {
    ...
    const responseLineString = `${this.request.version} ${code} ${message}\r\n`;
  ...
  public async defaultHeaders(): Promise<void> {
    ...
    // If there is the content type header, write it.
    if (this._contentType) {
      await this.header(HTTPHeaderType.ContentType, this._contentType.encode());
    }
  }
  ...
  public async header(key: string, value: string): Promise<void> {
    ...
    const headerStringBuffer = Buffer.from(headerString, "utf-8");
  ...
  protected async _startBody(): Promise<void> {
    ...
    if (shouldUseChunking) {
      ...
      const transform: Transform = new Transform({
        final: async (callback: (error?: Error | undefined) => void) => {
          // Performs some logging.
          this.session.shouldTrace(() =>
            this.session.trace(
      ...
      // Creates the transform stream.
      const transform: Transform = new Transform({
        final: async (callback: (error?: Error | null) => void): Promise<void> => {
          // Ends the body.
          await this.endBody();
   ...
   */
  public async sendBufferedBody(buffer: Buffer): Promise<void> {
    ...
    // Wait for either the socket to close or an error to occur.
   ...
   */
  public async sendStreamedBody(readable: Readable): Promise<void> {
    // Checks if we're allowed to do this.
    if (this._state !== HTTPResponseState.WritingResponseHeaders)
      throw new Error(
    ...
    // Waits for either an error or the socket to close.
  ...
  protected static _httpContentTypeFromFileExtension(
    ...
    switch (extension) {
      case ".html":
        return HTTPMediaType.TextHTML;
      case ".txt":
        return HTTPMediaType.TextPlain;
      case ".jpg":
        return HTTPMediaType.ImageJPEG;
      case ".css":
        return HTTPMediaType.TextCSS;
      case ".js":
        return HTTPMediaType.TextJavascript;
      case ".mp4":
        return HTTPMediaType.VideoMP4;
      default:
        return HTTPMediaType.OctetStream;
    }
  }
   ...
   * @param mediaType the type of media (for "Content-Type" header).
   */
  public async buffer(
    buffer: Buffer,
    status = 200,
    mediaType = HTTPMediaType.TextPlain
  ): Promise<void> {
    // Performs some logging.
    this.session.shouldTrace(() =>
      this.session.trace(
        `buffer(): status code ${status}, size: ${buffer.length}, contentType: '${mediaType}'`
      )
    );
   ...
   */
  public async text(text: string, status = 200): Promise<void> {
    // Stringifies the object, and creates the buffer version.
    const buffer: Buffer = Buffer.from(text, "utf-8");

    // Writes the buffer to the client.
    await this.buffer(buffer, status, HTTPMediaType.TextPlain);
  }
   ...
   */
  public async html(text: string, status = 200): Promise<void> {
    // Stringifies the object, and creates the buffer version.
    const buffer: Buffer = Buffer.from(text, "utf-8");

    // Writes the buffer to the client.
    await this.buffer(buffer, status, HTTPMediaType.TextHTML);
  }
   ...
   */
  public async json(object: any, status = 200): Promise<void> {
    // Stringifies the object, and creates the buffer version.
    const stringified: string = JSON.stringify(object);
    const buffer: Buffer = Buffer.from(stringified, "utf-8");

    // Writes the buffer to the client.
    await this.buffer(buffer, status, HTTPMediaType.ApplicationJson);
  }
  ...
  public async render(
    ...
    const templatingFunction = (consolidate as any)[engine];
   ...
   * @returns the current instance.
   */
  public async file(filePath: string, status = 200): Promise<void> {
    ...
    this.session.shouldTrace(() =>
      this.session.trace(
        `file(): writing file '${filePath}', status code ${status}, size: ${stats.size}, type (MIME): ${mediaType}`
      )
    );
